...
public:
	BinaryTreeNode<T>* FindSameAcsetor(Node* node1,Node* node2)
	{
		int tmp;
		stack<Node *> s1;
		stack<Node *> s2;
		FindNode(_root, node1, s1);
		FindNode(_root, node2, s2);
		if (s1.size() > s2.size())
		{
			tmp = s1.size() - s2.size();
			for (int i = 0; i < tmp; ++i)
	...
	bool FindNode(Node *root, Node *x, stack<Node *>&s)
	{
		if (root == NULL)
		{
			return;
		...
		if (FindNode(root->_left, s))
		{
			return true;
		}
		if (FindNode(root->_right, s))
		{
			return true;
		}
		s.pop();
