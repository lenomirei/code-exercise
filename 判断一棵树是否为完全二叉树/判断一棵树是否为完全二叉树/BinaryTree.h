...
public:
	...
	bool _Judge(Node *root)
	{
		if (root == NULL)
		{
			return true;
		}
		int left = _GetLength(root->_left);
		int right = _GetLength(root->_right);
		...
		return _Judge(root->_left) && _Judge(root->_right);
	}
	int _GetLength(Node *node)
	{
		if (node == NULL)
		{
			return 0;
		}
		int left = _GetLength(node->_left);
		int right = _GetLength(node->_right);
...
{
	int a[8] = {1,2,3,'#','#','#',4,5};
	BinaryTree<int> t1(a, 8);
	cout << t1.IsPerfectTree() << endl;
}