...
public:
	Node * ChangetoList()
	{
		...
		ChangetoList_core(_root, prev);
		return cur;
	}

	void ChangetoList_core(Node *root,Node *&prev)
	{
		if (root == NULL)
		{
			return;
		}
		ChangetoList_core(root->_left, prev);
		if (prev)
		{
			prev->_right = root;
		}
		root->_left = prev;
		prev = root;
		ChangetoList_core(root->_right, prev);
	}
...
//	bst1.InOrder();
	...
	//}
	//bst1.InOrder();
	BSTNode<int,int>* cur=bst1.ChangetoList();
	while (cur)
	{
		cout << cur->_key << "   ";
		cur = cur->_right;
